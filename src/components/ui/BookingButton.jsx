...
const BookingButton = ({ className = '', label = 'Book a Consultation' }) => {
  const buttonContainerRef = useRef(null);
  const navigate = useNavigate();
  
  useEffect(() => {
    // Load the Google Calendar CSS if not already loaded
    ...
    // Load the Google Calendar script if not already loaded
    if (!document.querySelector('script[src="https://calendar.google.com/calendar/scheduling-button-script.js"]')) {
      ...
      scriptElement.onload = initializeButton;
    } else {
      // If script is already loaded, initialize button directly
      initializeButton();
    }
    
    function initializeButton() {
      // Wait for the calendar object to be available
      const checkCalendarInterval = setInterval(() => {
      ...
      setTimeout(() => clearInterval(checkCalendarInterval), 10000);
    }
    
    // No cleanup needed as we want to keep the scripts loaded for other components
  }, [label, navigate]);
  
  return (
    <div ref={buttonContainerRef} className={`booking-button-container ${className}`}></div>
  );
};
